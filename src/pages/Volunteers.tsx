...
const Volunteers = () => {
  ...
  const fetchVolunteers = async () => {
    try {
      const data = useApi
        ? await api.volunteers.list()
        : (await supabase.from("volunteers").select("*").order("rating", { ascending: false })).data;
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      if (useApi) {
        await api.volunteers.create({ user_id: user.id, skills: selectedSkills, bio: bio || undefined });
      } else {
